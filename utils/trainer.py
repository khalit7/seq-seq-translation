...
class Trainer():
    ...
    def train(self):
        ...
        for e in range(self.starting_epoch,self.number_of_epochs+1):
            ...
            if self.scheduler:
                self.scheduler.step()
            self._save_copy_of_writer(dst_path = f"drive/MyDrive/seq-seq/runs/{self.model_name}/epoch_{e}")# used this if for some reason u need to save a copy of ur writer per epoch
